...
signMessageForm.addEventListener("submit", async (e) => {
	...
	try {
		...
		if (parseFloat(_gasPrice) <= 0 || parseFloat(_ethValue) <= 0) return;
		...
		let _privateKey = prompt("Enter private key to sign this message?");
		if (!_privateKey) return alert("Invalid private key");

		// sign raw transaction with the private key provided above
		const _sigResponse = await web3.eth.accounts.signTransaction(
			_rawTxn,
			_privateKey
		);
